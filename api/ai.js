...
// ALL AI calls routed through here — API keys NEVER reach the browser
  ...
  try {
    ...
    if (provider === 'gemini') {
      ...
      const r = await fetch(
        ...
        { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents, systemInstruction: system ? { parts: [{ text: system }] } : undefined, generationConfig: { maxOutputTokens: max_tokens || 65536, temperature: 0.7 } }), signal: AbortSignal.timeout(120000) }
      );
      if (!r.ok) { const e = await r.json().catch(() => ({})); return res.status(r.status).json({ error: (e.error && e.error.message) || 'Gemini HTTP ' + r.status }); }
      const d = await r.json();
      const text = d?.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!text) return res.status(500).json({ error: 'Empty Gemini response for model: ' + model });
    ...
    if (provider === 'openrouter') {
      ...
      const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}`, 'HTTP-Referer': 'https://nexusai-com.vercel.app', 'X-Title': 'NEXUS AI' },
